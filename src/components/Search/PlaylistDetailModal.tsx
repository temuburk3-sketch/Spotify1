...
import { 
  X, Play, Plus, ListPlus, Check, HardDrive, 
  Loader2, Disc, BookmarkCheck, Heart, Sparkles, Music 
} from 'lucide-react';
...
interface PlaylistDetailModalProps {
  ...
  onSavePlaylistToLibrary?: (name: string, tracks: Track[]) => void;
...
export const PlaylistDetailModal: React.FC<PlaylistDetailModalProps> = ({
  ...
  useEffect(() => {
    ...
    const alreadyExists = userPlaylists.some(
      p => p.name.toLowerCase().trim() === playlist.name.toLowerCase().trim()
    );
    setIsSaved(alreadyExists);

    // If it's a user playlist, we already have tracks
  ...
  const handlePlayAll = () => {
    ...
    onPlayTrack(tracks[0], tracks, playlist.name);
  };

  const handleSaveToLibrary = () => {
    if (tracks.length === 0 || !onSavePlaylistToLibrary) return;
    onSavePlaylistToLibrary(playlist.name, tracks);
    setIsSaved(true);
  };
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-6 bg-black/85 backdrop-blur-xl animate-fade-in select-none">
      <div className="w-full max-w-3xl max-h-[90vh] bg-[#0c0f17] border border-neutral-800 rounded-3xl overflow-hidden shadow-2xl flex flex-col relative text-neutral-100">
          ...
          <div className="flex flex-col sm:flex-row items-center sm:items-end gap-5">
            <div className="relative group shrink-0">
              <img
                ...
                alt={playlist.name}
                className="w-28 h-28 sm:w-36 sm:h-36 rounded-2xl object-cover shadow-2xl border border-neutral-700/80"
              />
            </div>
            ...
            <div className="flex-1 text-center sm:text-left space-y-2">
              <div className="flex items-center justify-center sm:justify-start gap-2">
                <span className="px-2.5 py-0.5 rounded-full bg-emerald-500/20 text-emerald-400 border border-emerald-500/30 text-[11px] font-black uppercase tracking-wider flex items-center gap-1">
                  <Music className="w-3.5 h-3.5" /> Çalma Listesi
                </span>
                {playlist.author && (
                  <span className="text-xs text-neutral-400 font-medium">
                    • Derleyen: {playlist.author}
                  </span>
                )}
              </div>
              ...
              <p className="text-xs text-neutral-400 line-clamp-2 max-w-xl">
                {playlist.description || 'Seçilmiş hit parçalar ve özel derlemeler.'}
              </p>

              <div className="flex items-center justify-center sm:justify-start gap-3 text-xs text-neutral-400">
                <span className="font-semibold text-neutral-300">{tracks.length > 0 ? tracks.length : playlist.trackCount} Şarkı</span>
                <span>•</span>
                <span>Yüksek Kalite Ses</span>
              </div>
              ...
              <div className="flex flex-wrap items-center justify-center sm:justify-start gap-2.5 pt-2">
                ...
                {onSavePlaylistToLibrary && (
                  <button
                    ...
                    disabled={isSaved || tracks.length === 0}
                    className={`px-4 py-2.5 rounded-full font-bold text-xs border transition flex items-center gap-1.5 cursor-pointer ${
                      isSaved
                        ? 'bg-neutral-900 border-emerald-500/40 text-emerald-400 cursor-default'
                        : 'bg-neutral-900 hover:bg-neutral-800 text-white border-neutral-700'
                    }`}
                  >
                    {isSaved ? (
                      <>
                        <BookmarkCheck className="w-4 h-4 text-emerald-400" /> Kütüphanene Eklendi
                      </>
                    ) : (
                      <>
                        <ListPlus className="w-4 h-4 text-emerald-400" /> Kütüphaneme Kaydet
                      </>
                    )}
                  </button>
                )}
        ...
        <div className="flex-1 overflow-y-auto p-4 sm:p-6 space-y-2 custom-scrollbar">
          <div className="flex items-center justify-between pb-2 mb-2 border-b border-neutral-800/80">
            <h2 className="text-xs font-bold text-neutral-400 uppercase tracking-wider flex items-center gap-2">
              ...
              <span>Liste Parçaları ({tracks.length})</span>
            </h2>
            <span className="text-[11px] text-neutral-500">Kesintisiz Akış</span>
          </div>

          {isLoading ? (
            <div className="py-20 text-center space-y-3">
              <Loader2 className="w-8 h-8 text-emerald-400 animate-spin mx-auto" />
              <p className="text-xs text-neutral-400 font-medium">Şarkılar yükleniyor...</p>
            </div>
          ) : tracks.length === 0 ? (
            <div className="py-16 text-center text-neutral-500 space-y-2">
              <Disc className="w-10 h-10 mx-auto opacity-40" />
              <p className="text-sm font-semibold">Bu çalma listesinde şarkı bulunamadı</p>
            </div>
          ) : (
              ...
              return (
                ...
                >
                  <div className="flex items-center gap-3 min-w-0 flex-1">
                    ...
                    <button
                      onClick={() => onPlayTrack(track, tracks, playlist.name)}
                    ...
                    >
                      ...
                      <div className="text-[11px] text-neutral-400 truncate flex items-center gap-1.5 mt-0.5">
                        ...
                        <span>•</span>
                        <span className="truncate text-neutral-500">{track.album || track.genre || 'Müzik'}</span>
                      </div>
                    </div>
                  </div>
        ...
        {/* Add Track to User Playlist Sub-Modal */}
        {selectedTrackForAdd && (
